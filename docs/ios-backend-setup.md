...
Before using a staging URL, run the local static and migration gate checks:

```text
npm run test:sql:static
npm run test:sql -- --strict
```

The strict gate requires a local Supabase CLI/database and
`ROOTINE_UPGRADE_DB_URL` for the disposable staging-copy upgrade. It never
creates or reads a live project when those inputs are absent; the protected
release workflow supplies them outside the repository.
