-- The device_id OUT parameter also collides with the ON CONFLICT target in
-- the six-argument B09 registration function. Prefer SQL column names for
-- any remaining unqualified identifiers in that function.
...
begin
  ...
  values (
    current_user_id, normalized_device_id, normalized_platform, normalized_app_version,
    normalized_environment,
    case when normalized_permission = 'denied' then null else normalized_push_token end,
    normalized_permission, now_utc, null, null, null, now_utc
  )
