...
type Task = {
  ...
  time?: string; tags?: string[]; list?: string;
  view: string; priority?: Priority; notes?: string;
...
type TagItem  = { id: string; label: string; color: string };
...
function DatePickerPopup({
  value, onConfirm, onClose, anchorEl,
}: {
  value: DateVal;
  ...
  // Position popup anchored below the trigger button
  ...
  const handleOk = () => {
    onConfirm({ date: selDate, time, reminder, repeat, startTime, endTime });
    onClose();
  };
  ...
  return (
    <div
      ref={popRef}
      style={{
        position: "fixed", top, left, width: `${popWidth}px`, zIndex: 9999,
        background: "#2A2A2A",
        border: `1px solid ${C.borderStrong}`,
        borderRadius: "14px",
          ...
          {[
            {
              ...
              content: <TimePicker value={time} onChange={setTime} />,
            },
            {
            ...
            return (
              <div key={field}>
                <div style={{ display: "flex", alignItems: "center", gap: "7px", padding: "9px 12px" }}>
                  ...
                  <button
                    onClick={() => setOpenTimeField(open ? null : field)}
                    style={{
                      flex: 1, padding: "6px 10px", borderRadius: "8px", textAlign: "center" as const,
                      background: open ? C.iceBlueBg : C.inputBg,
                      border: `1px solid ${open ? "rgba(71,114,250,0.35)" : C.borderSubtle}`,
                      color: open ? C.iceBlue : timeVal ? C.textPrimary : C.textDisabled,
                ...
                {open && !allDay && (
                  <div style={{ padding: "0 12px 8px" }}>
                    <TimePicker
                      value={timeVal}
                      onChange={v => field === "start" ? setStartTime(v) : setEndTime(v)}
                    />
                  </div>
                )}
              </div>
...
// ── Task row ──────────────────────────────────────────────
function TaskRow({
  task, selected, onToggle, onSelect, tagi,
}: {
  ...
  const pColor = task.priority ? PRIORITY_COLOR[task.priority] : null;
  ...
  return (
    ...
    >
      <div className="mt-[3px] flex-shrink-0" style={{ width: "6px" }}>
        {pColor && !task.done && (
          <div className="w-1.5 h-1.5 rounded-full mt-[3px]" style={{ background: pColor }} />
        )}
      </div>
      <div
        onClick={e => { e.stopPropagation(); onToggle(task.id); }}
        className="mt-[1px] w-[16px] h-[16px] rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 cursor-pointer"
        style={{
          border: `1.5px solid ${task.done ? C.seaGlass : C.borderStrong}`,
          background: task.done ? C.seaGlassBg : "transparent",
        }}
      >
        {task.done && <Check size={8} strokeWidth={2.5} style={{ color: C.seaGlass }} />}
      </div>
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {task.time && (
              <div className="flex items-center gap-1">
                ...
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "10px", color: C.iceBlue }}>
                  {task.time}
                </span>
              </div>
            )}
            {taskTags.map(td => (
              <span key={td.id} className="text-[10px] px-1.5 py-[2px] rounded-md font-medium"
                style={{ color: td.color, background: td.color + "18" }}>
                #{td.label}
              </span>
            ))}
          </div>
...
function TaskDetail({ task, onClose, onUpdate, onDelete, listy, tagi }: {
  ...
  const [taskDateVal,    setTaskDateVal]    = useState<DateVal>({
    date: null, time: task.time ?? "", reminder: "", repeat: "", startTime: "09:00", endTime: "10:00",
  });
...
function SummaryPanel({ tasks, habits, onToggleHabit }: {
  ...
  return (
    <div className="flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden px-4 py-5 space-y-6">
      <section>
        <p className="text-[10px] uppercase tracking-widest font-semibold mb-3" style={{ color: C.textMuted }}>Podsumowanie dnia</p>
        <div className="grid grid-cols-3 gap-1.5 mb-3">
          {[
            { label: "Wykonane",  val: String(done),              accent: done > 0 ? C.seaGlass : C.textPrimary },
            { label: "Pozostałe", val: String(total - done),      accent: C.textPrimary },
            { label: "Nawyki",    val: `${doneH}/${habits.length}`,accent: C.textPrimary },
          ].map((s, i) => (
            <div key={i} className="rounded-xl p-2.5 text-center" style={{ background: C.card, border: `1px solid ${C.borderSubtle}` }}>
              <div className="text-[20px] font-semibold leading-none" style={{ fontFamily: "'DM Mono',monospace", color: s.accent }}>{s.val}</div>
              <div className="text-[9px] mt-1.5 uppercase tracking-widest" style={{ color: C.textDisabled }}>{s.label}</div>
            </div>
          ))}
        </div>
        <div className="rounded-xl p-3.5" style={{ background: C.card, border: `1px solid ${C.borderSubtle}` }}>
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-[11px]" style={{ color: C.textMuted }}>Postęp dnia</span>
            <span className="text-[11px] font-medium" style={{ fontFamily: "'DM Mono',monospace", color: pct === 100 ? C.seaGlass : C.iceBlue }}>{pct}%</span>
          </div>
          ...
          {total > 0 && (
            <div className="flex gap-0.5 mt-2.5">
              {tasks.map(t => <div key={t.id} className="flex-1 h-[3px] rounded-full" style={{ background: t.done ? C.seaGlass : C.borderStrong }} />)}
            </div>
          )}
        </div>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] uppercase tracking-widest font-semibold" style={{ color: C.textMuted }}>Nawyki na dziś</p>
          <span className="text-[10px]" style={{ fontFamily: "'DM Mono',monospace", color: C.textDisabled }}>{doneH}/{habits.length}</span>
        </div>
        <div className="space-y-1.5">
          {habits.map(h => (
            <div key={h.id} onClick={() => onToggleHabit(h.id)}
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[12px] leading-none" style={{ color: h.done ? C.textDisabled : C.textSecond, textDecoration: h.done ? "line-through" : "none" }}>{h.name}</div>
                {h.streak > 0 && (
                  <div className="flex items-center gap-1 mt-0.5">
                    <Flame size={9} strokeWidth={1.5} style={{ color: C.warning }} />
                    <span className="text-[10px]" style={{ color: C.textDisabled }}>{h.streak} dni</span>
                  </div>
                )}
              </div>
      ...
      <section>
        <p className="text-[10px] uppercase tracking-widest font-semibold mb-3" style={{ color: C.textMuted }}>Ten tydzień</p>
        <div className="rounded-xl p-3.5" style={{ background: C.card, border: `1px solid ${C.borderSubtle}` }}>
              ...
              <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                <div className="w-full flex items-center justify-center rounded-lg text-[10px] font-medium transition-all"
                  style={{
                    ...
                    color: d.today ? C.iceBlue : i < todayIdx ? C.seaGlass : C.textDisabled,
                    border: `1px solid ${d.today ? "rgba(71,114,250,0.3)" : "transparent"}`,
                  }}>
                  {i < todayIdx ? <Check size={8} strokeWidth={2.5} /> : d.n}
                </div>
                <span className="text-[9px]" style={{ color: d.today ? C.iceBlue : C.textDisabled }}>{DL[i]}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 pt-3 border-t flex items-center justify-between" style={{ borderColor: C.borderSubtle }}>
            <span className="text-[11px]" style={{ color: C.textMuted }}>Seria aktywna</span>
            <div className="flex items-center gap-1">
...
export default function Zadania() {
  ...
  const addTask = () => {
    ...
    setTasks(p => [...p, {
      id, text, done: false, view: taskView === "wszystkie" || taskView === "podsumowanie" ? "dzis" : taskView,
      tags: newTaskTags.length > 0 ? newTaskTags : undefined,
      list: newTaskList ?? undefined,
      priority: newPriority ?? undefined,
      time: newDateVal.time || undefined,
  ...
  const deleteTask = (id: number) => { setTasks(p => p.filter(t => t.id !== id)); setSelectedId(null); };
  ...
  return (
    <div className="flex flex-1 overflow-hidden h-full">
      ...
      {/* ── Sub-sidebar ── */}
      <div className="w-[200px] flex-shrink-0 border-r flex flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
            ...
            return (
              <button key={v.id}
                ...
                {v.id !== "podsumowanie" && count > 0 && (
                  <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "10px", color: active ? C.iceBlue : C.textDisabled }}>{count}</span>
                )}
              </button>
            );
          })}
              ...
              return (
                <button key={p}
                  ...
                  style={{
                    ...
                    color: active ? color : C.textDisabled,
                    border: `1px solid ${active ? color + "55" : C.borderSubtle}`,
                    cursor: "pointer",
                  }}>
                  {label}
                </button>
        ...
        <div className="px-2.5 mb-2">
          <div className="flex items-center justify-between px-1.5 mb-1.5">
            ...
            {listyOpen && (
              <button onClick={() => { setAddingList(true); setAddingTag(false); }}
                ...
                <div key={l.id} className="group relative">
                  {editingListId === l.id ? (
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-[7px] rounded-lg text-[12.5px] transition-all"
                      style={{
                      ...
                      {count > 0 && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "10px", color: active ? l.color : C.textDisabled }}>{count}</span>}
                    </button>
                  )}
                  {/* Hover actions */}
                  {editingListId !== l.id && (
        ...
        <div className="px-2.5 mb-2">
          <div className="flex items-center justify-between px-1.5 mb-1.5">
            ...
            {tagiOpen && (
              <button onClick={() => { setAddingTag(true); setAddingList(false); }}
                style={{ background: "none", border: "none", cursor: "pointer", color: C.textDisabled, display: "flex", padding: 2 }}
                onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = C.textMuted)}
                onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = C.textDisabled)}>
                <Plus size={11} strokeWidth={2} />
        ...
        <div className="px-2.5 py-3 border-t space-y-px" style={{ borderColor: C.borderSubtle }}>
          {([
            ...
            { icon: Trash2,    label: "Kosz",      view: null },
          ] as const).map(({ icon: Icon, label, view }) => {
            const active = view && taskView === view && !listFilter && !tagFilter;
            return (
              <button key={label}
                onClick={() => { if (view) { setTaskView(view); setListFilter(null); setTagFilter(null); } }}
                className="w-full flex items-center gap-2.5 px-3 py-[7px] rounded-lg text-[12px] transition-colors"
                style={{
      ...
      {taskView === "podsumowanie" && <SummaryDocument tasks={tasks} listy={listy} />}
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0"
        ...
        <div className="flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden px-3 py-3">
          {/* Add task input */}
          <div className="mx-1 mb-3 rounded-xl transition-all duration-200"
            style={{
              background: C.inputBg,
              border: `1px solid ${inputFocused ? "rgba(71,114,250,0.5)" : C.borderSubtle}`,
              boxShadow: inputFocused ? "0 0 0 3px rgba(71,114,250,0.08)" : "none",
            }}>
      ...
      {/* ── Right panel ── */}
      <div className="w-[288px] flex-shrink-0 border-l flex flex-col overflow-hidden"
        style={{ background: C.subSidebar, borderColor: C.borderSubtle }}>
        {selectedTask ? (
