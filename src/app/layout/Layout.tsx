...
import {
  LayoutGrid, CheckSquare, CalendarDays, Target, Dumbbell, Salad,
...
export default function Layout() {
  ...
  return (
    ...
    >
      <aside
        className="app-sidebar w-[204px] flex-shrink-0 flex flex-col border-r"
      ...
      >
        ...
        <div className="px-5 pt-6 pb-5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="app-brand-label text-[13px] font-semibold tracking-wide" style={{ color: T.primaryText }}>
              Routine
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 px-2.5 space-y-px overflow-y-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {NAV.map(({ label, icon: Icon, to }) => (
            <NavLink key={to} to={to} end={to === "/"}>
              {({ isActive }) => (
                <div
                  className="app-nav-item w-full flex items-center gap-2.5 px-3 py-[7px] rounded-lg text-[11px] transition-all duration-150"
                  style={{
                ...
                >
                  ...
                  <span className="app-nav-label leading-none">{label}</span>
                </div>
              )}
            </NavLink>
          ))}
        ...
        <div className="px-4 py-3.5 border-t flex items-center gap-2.5" style={{ borderColor: T.sidebarBorder }}>
          ...
          <span className="app-user-name text-[11px] font-medium flex-1" style={{ color: "#A0A0A0" }}>Mateusz</span>
