...
export function SportTemplates({
  ...
  return (
    <section className="sport-record-view sport-templates-view" aria-labelledby="sport-templates-title">
      <div className="sport-record-view__intro"><div><h2 id="sport-templates-title">Szablony</h2><p>Powtarzalne definicje treningów. Zawartość jest widoczna od razu, bez otwierania edycji.</p></div><Button variant="primary" leadingIcon={<Plus size={14} />} onClick={() => setCreating(true)}>Nowy szablon</Button></div>
      ...
      {filtered.length === 0 ? <EmptyState title="Brak pasujących szablonów" description="Zmień wyszukiwanie albo utwórz nowy szablon." action={<Button variant="quiet" onClick={() => setCreating(true)}>Nowy szablon</Button>} /> : groups.filter((group) => filtered.some((template) => template.discipline === group)).map((group) => (
...
function TemplateSeriesRow({ series, index, metricMode, onUpdate, onRemove }: {
  ...
  return (
    <div className="sport-template-editor__series-row">
      ...
      <Button variant="ghost" size="sm" aria-label={`Usuń serię ${index + 1}`} disabled={index === 0} onClick={onRemove}><Trash2 size={13} /></Button>
    </div>
  );
}
  ...
  return (
    <div className={`sport-template-editor__item-wrap ${expanded ? "is-expanded" : ""} ${dropTarget ? "is-drop-target" : ""} ${isDragging ? "is-dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); onSectionDragOver(); }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); onDrop(); }}>
      <div className="sport-template-editor__item">
        ...
        <span className="sport-template-editor__drag-handle" draggable aria-label="Przeciągnij, aby zmienić kolejność" onDragStart={(event) => { event.stopPropagation(); onDragStart(); }} onDragEnd={onDragEnd}><GripVertical size={14} aria-hidden="true" /></span>
        ...
        <div className="sport-template-editor__item-actions"><Button variant="ghost" size="sm" aria-label="Duplikuj element" onClick={onDuplicate}><Copy size={13} /></Button><Button variant="ghost" size="sm" aria-label="Usuń element" onClick={onRemove}><Trash2 size={13} /></Button></div>
      </div>
      {expanded && (
...
function TemplateEditor({ template, exercises, onClose, onSave, onCreateExercise }: { template?: WorkoutTemplate; exercises: Exercise[]; onClose: () => void; onSave: (template: WorkoutTemplate) => void; onCreateExercise?: () => void }) {
  ...
  return (
    <Modal title={template ? "Edytuj szablon" : "Nowy szablon"} eyebrow="Pełny edytor zawartości" description="Parametry zapisane w szablonie są niezależne od domyślnych parametrów biblioteki ćwiczeń." width={920} onClose={onClose} footer={<><Button variant="ghost" onClick={onClose}>Anuluj</Button><Button variant="primary" onClick={submit}>Zapisz szablon</Button></>}>
      <div className="sport-template-editor">
        ...
        <div className="sport-template-editor__toolbar">
          <div className="sport-template-editor__actions"><Button variant="quiet" leadingIcon={<Plus size={13} />} onClick={() => openPicker()}>Dodaj ćwiczenie</Button><Button variant="ghost" onClick={onCreateExercise}>Utwórz nowe ćwiczenie</Button><Button variant="ghost" size="sm" onClick={toggleAllItems}>{items.some((item) => item.exerciseId) && items.filter((item) => item.exerciseId).every((item) => expandedItemIds.includes(item.id)) ? "Zwiń wszystkie" : "Rozwiń wszystkie"}</Button></div>
          ...
          return <section key={section.id} className={`sport-template-editor__section ${sectionCollapsed ? "is-collapsed" : ""} ${sectionDropTarget ? "is-drop-target" : ""} ${draggedSectionId === section.id ? "is-dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); if (draggedSectionId && draggedSectionId !== section.id) setDropTargetSectionId(section.id); else if (draggedId) setDropTargetSectionId(section.id); }} onDrop={(event) => { event.preventDefault(); if (draggedSectionId) moveSection(draggedSectionId, section.id); else if (draggedId) moveItem(draggedId, section.id); clearDragState(); }}>
            <header>
              <div className="sport-template-editor__section-heading">
                <span className="sport-template-editor__section-drag-handle" draggable aria-label={`Przeciągnij sekcję ${section.name}`} onDragStart={(event) => { event.stopPropagation(); setDraggedSectionId(section.id); setDraggedId(null); setDropTargetSectionId(null); }} onDragEnd={clearDragState}><GripVertical size={15} aria-hidden="true" /></span>
            ...
            {!sectionCollapsed && <div className="sport-template-editor__section-actions"><Button variant="quiet" size="sm" leadingIcon={<Plus size={12} />} onClick={() => openPicker(section.id)}>Dodaj ćwiczenie</Button></div>}
          </section>;
        })}
        {error && <p className="sport-planner-form__error" role="alert">{error}</p>}
      </div>
      {pickerOpen && <Modal title="Dodaj ćwiczenie" eyebrow="Picker biblioteki" description="Wybierz ćwiczenie. Jego domyślne parametry zostaną skopiowane do tego szablonu." width={620} onClose={() => setPickerOpen(false)} footer={<Button variant="ghost" onClick={onCreateExercise}>Utwórz nowe ćwiczenie</Button>}><Input label="Szukaj" value={pickerQuery} data-autofocus onChange={(event) => setPickerQuery(event.target.value)} /><div className="sport-exercise-picker">{filteredPicker.map((exercise) => <button key={exercise.id} type="button" onClick={() => chooseExercise(exercise)}><span><strong>{exercise.name}</strong><small>{exercise.primaryMuscle} · {exercise.equipment.join(", ") || "Bez sprzętu"}</small></span><Plus size={14} /></button>)}</div></Modal>}
    </Modal>
  );
}
