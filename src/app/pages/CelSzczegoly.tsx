      ...
      <header className="border-b px-8 pb-0 pt-6" style={{ borderColor: C.border }}>
        <div className="mb-5 flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl border" style={{ color: goal.color, background: `${goal.color}16`, borderColor: `${goal.color}35` }}>{goal.customIcon ? <img src={goal.customIcon} alt="" className="h-8 w-8 object-contain" /> : <Target size={21} strokeWidth={1.6} />}</div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><h1 className="text-[21px] font-semibold" style={{ color: C.primary }}>{goal.title}</h1><span className="rounded-md border px-2 py-1 text-[9px]" style={{ color: goal.color, borderColor: `${goal.color}40`, background: `${goal.color}0A` }}>{statusLabels[goal.status]}</span></div>
              <p className="mt-1 max-w-2xl text-[11px] leading-5" style={{ color: C.muted }}>{goal.description || "Brak opisu celu"}</p>
              <div className="mt-2 flex flex-wrap gap-3 text-[9px]" style={{ color: C.muted }}><span style={{ color: category?.color }}>{category?.label}</span><span>•</span><span>Termin: {fmtDate(goal.dueDate)}</span><span>•</span><span>{getGoalMetric(goal)}</span></div>
            </div>
          </div>
