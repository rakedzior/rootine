...
import { Badge, Button, Card, EmptyState as UiEmptyState, Menu, MenuItem, ModuleMain, ModuleShell, PageHeader, Tabs, uiColors } from "../ui";

const C = {
  bg: uiColors.graphiteCanvas, panel: uiColors.graphiteCard, input: uiColors.graphiteInput, border: uiColors.borderSubtle, strong: uiColors.borderStrong,
  primary: uiColors.chalkWhite, second: uiColors.textSecondary, muted: uiColors.textMuted, blueText: uiColors.precisionBlueText, green: uiColors.success, warning: uiColors.warning, danger: uiColors.danger,
};
...
const statusColor = (goal: { status: GoalStatus; health: "ontrack" | "risk" }) => {
  ...
  return goal.health === "risk" ? C.warning : C.blueText;
};
...
export default function CelSzczegoly() {
  ...
  if (!goal) {
    return <div className="flex flex-1 flex-col items-center justify-center gap-4" style={{ background: C.bg, color: C.second }}><Target size={38} strokeWidth={1.2} /><h1 className="text-[22px] font-semibold" style={{ color: C.primary }}>Nie znaleziono celu</h1><Button variant="primary" onClick={() => navigate("/cele")}>Wróć do celów</Button></div>;
  }
        ...
        <div className="mx-auto max-w-[1100px]">
          ...
          {tab === "overview" && <div className="goal-full-overview grid grid-cols-[minmax(0,1fr)_320px] gap-5">
            <div className="space-y-5">
              <Card as="section" padding="spacious"><div className="mb-4 flex items-end justify-between"><div><p className="text-[10px] uppercase tracking-wider" style={{ color: C.muted }}>Ogólny postęp</p><p className="mt-2 text-[22px] font-semibold" style={{ color: C.blueText, fontFamily: "var(--font-data)" }}>{progress}%</p></div><p className="text-[11px]" style={{ color: C.second }}>{getGoalMetric(goal)}</p></div><div className="h-2.5 overflow-hidden rounded-full" style={{ background: C.strong }}><div className="h-full rounded-full" style={{ width: `${progress}%`, background: goal.color }} /></div></Card>
              {goal.progressMode === "milestones" ? (
                <section>
                  <div className="mb-3 flex items-center justify-between"><h2 className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: C.muted }}>Najbliższe kamienie milowe</h2><button type="button" onClick={() => setTab("progress")} className="text-[10px]" style={{ color: C.blueText }}>Zobacz wszystkie</button></div>
