...
const C = {
  ...
  primary: uiColors.chalkWhite, second: uiColors.textSecondary, muted: uiColors.textMuted, blue: uiColors.precisionBlue, green: uiColors.success, warning: uiColors.warning, danger: uiColors.danger,
};
    ...
    <ModuleShell>
      ...
      <PageHeader
        ...
        actions={<>
          ...
          <div className="relative"><Button variant="ghost" iconOnly aria-label="Więcej opcji" onClick={() => setMenuOpen((open) => !open)}><Ellipsis size={15} /></Button>{menuOpen && <Menu className="absolute right-0 top-11 z-30 w-44"><MenuItem onClick={() => { store.updateGoal(goal.id, { status: goal.status === "archived" ? "active" : "archived" }); setMenuOpen(false); }} leadingIcon={goal.status === "archived" ? <RotateCcw /> : <Archive />}>{goal.status === "archived" ? "Przywróć" : "Archiwizuj"}</MenuItem><MenuItem onClick={() => { store.duplicateGoal(goal.id); setMenuOpen(false); }} leadingIcon={<Plus />}>Duplikuj</MenuItem><MenuItem tone="danger" onClick={() => { setDeleteGoalOpen(true); setMenuOpen(false); }} leadingIcon={<Trash2 />}>Usuń</MenuItem></Menu>}</div>
        </>}
        below={<Tabs ariaLabel="Sekcje celu" activeId={tab} onChange={(id) => setTab(id as TabId)} items={tabs.map(({ id, label, icon: Icon, count }) => ({ id, label: <span className="flex items-center gap-1.5"><Icon size={12} />{label}{count !== undefined && <span className="rounded px-1.5 py-0.5 text-[9px]" style={{ background: C.input }}>{count}</span>}</span> }))} />}
      />
