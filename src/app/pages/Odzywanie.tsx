        ...
        <div className="rounded-2xl p-4 mb-4 flex items-center gap-4" style={{ background: C.card, border: `1px solid ${C.borderSubtle}` }}>
          ...
          <div className="flex gap-1.5 flex-1">
            {Array.from({ length: WATER_MAX }).map((_, i) => (
              <div key={i} className="h-4 flex-1 rounded-md transition-all" style={{ background: i < WATER ? C.iceBlue : C.inputBg, opacity: i < WATER ? 0.8 : 1 }} />
            ))}
          </div>
                ...
                <div className="px-4 py-2">
                  {meal.items.map((item, i) => (
                    <div key={i} className="flex items-center gap-2 py-1.5">
                      <div className="w-1 h-1 rounded-full" style={{ background: C.borderSubtle }} />
