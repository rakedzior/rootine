...
- Primary task: start from today's work, or select a project directly from the company tree, then create, edit, complete, nest, move, search, and delete tasks.
...
- Direction: a quiet, list-first workspace that shares the visual grammar of Zadania. Today is the default main screen. The local rail is split into `Przegląd` and `Firmy i projekty`, with a collapsible company → project tree; selecting a project opens its task outline directly in the canvas. Semantic color is limited to small labels and icons, never full task rows.
